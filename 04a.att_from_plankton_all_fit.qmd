...
subtitle: "Train a XGBoost model to predict POC attenuation from plankton diversity data."
...
output_filename <- "data/04.att_from_plankton_all_fit.Rdata"
```
...
exp_vars <- df_all %>% select(prop_crustacea:mo_eve) %>% colnames() # all variables
# Metadata
meta_vars <- c("lon", "lat")
```
...
-   80% of data for training. This subset is used for nested cross-validation with:

    -   20% of data for validation

    -   80% of data for learning

Two different methods for data splitting are used:

-   stratified on quantiles of the response variable
...
folds <- bind_rows(
  ...
  nested_cv(
    df_all,
    outside = vfold_cv(v = n_folds, strata = att, breaks = 4),
    inside = vfold_cv(v = n_folds, strata = att, breaks = 4)) %>%
