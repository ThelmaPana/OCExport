...
# Join together
df_mod <- df_100 %>% left_join(df_1000, by = join_by(lat, lon))
...
```{r}
df_argo_sl <- df_argo %>% filter(depth == 937.5)
ggmap(df_argo_sl, var = "poc_tot", type = "raster") + scale_fill_cmocean(name = "matter", na.value = NA)
```
...
```{r}
df_mod
unique(df_argo_sl$lon)
unique(df_argo_sl$lat)
...
toto <- df_mod %>% 
  ...
  group_by(lon, lat) %>% 
  summarise(poc_mod = mean(poc_mod), .groups = "drop")
unique(toto$lon_r)
unique(toto$lat_r)
ggmap(toto, var = "poc_mod", type = "raster")
```
