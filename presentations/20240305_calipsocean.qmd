...
# Model outputs
load(here("data/08.all_preds_poc_from_env.Rdata"))
...
::: {.fragment .fade-in-then-semi-out .absolute top="350"}
...
::: {.fragment .fade-in-then-semi-out .absolute top="450"}
-   Can [POC]{style="color: black;background-color: #00b5ff;border-radius: 10px"} be predicted from [plankton]{style="color: black;background-color: #5ed496;border-radius: 10px"} diversity?
:::

::: {.fragment .fade-in-then-semi-out .absolute top="550"}
-   Can [plankton]{style="color: black;background-color: #5ed496;border-radius: 10px"} diversity be predicted from [env]{style="color: black;background-color: #ffe50a;border-radius: 10px"}?
:::

::: {.absolute top="650"}
[\*plankton = zooplankton]{style="color: gray;font-size: 20px;"} [\*\*Here, we will use POC 1000 m as a proxy for carbon export.]{style="color: gray;font-size: 20px;"}
:::
...
```{r poc_map}
ggmap(df = df_c, var = "poc", type = "raster") + labs(fill = "POC flux 1000m\n(mmolC m⁻² y⁻¹)")
```
:::
...
::: aside
[*In situ* imaging, coherent dataset with \~30 taxa.]{style="color: gray;font-size: 20px;"}
:::
...
Functional diversity metrics [(Magneville et al. 2022)](https://nsojournals.onlinelibrary.wiley.com/doi/10.1111/ecog.05904)
:::
...
Training\* VS test set, stratified by [POC]{style="color: black;background-color: #00b5ff;border-radius: 10px"}.
...
all_preds %>% 
  ...
  #mutate(split = ifelse(split == "train", "test")) %>% 
  ggplot() +
  geom_polygon(data = world, aes(x = lon, y = lat, group = group), fill = "grey") +
  geom_point(aes(x = lon, y = lat, colour = split), size = 0.5) +
  scale_colour_manual(values = c("train" = "#004c96", "test" = "#ff8700")) +
  coord_quickmap(expand = FALSE)
```

::: aside
[\*training = learning + validation, cross-validated]{style="color: gray;font-size: 20px;"}
:::
:::
...
::: {.fragment .fade-in}
```{r poc_from_plankton}
load(here("data/09.all_preds_poc_from_plankton.Rdata"))
...
-   [ta. richness ×2]{style="font-size: 30px;"}
...
load(here("data/10.all_preds_poc_from_plankton_best.Rdata"))
...
::: aside
[Grey ribbon = sd across *cp* profiles.]{style="color: gray;font-size: 20px;"}
:::
...
::: {.fragment .fade-in}
```{r plankton_from_env}
load(here("data/12.all_preds_plankton_best_from_env.Rdata"))
...
::: {.fragment .fade-in}
-   Can [POC]{style="color: black;background-color: #00b5ff;border-radius: 10px"} be predicted from [plankton]{style="color: black;background-color: #5ed496;border-radius: 10px"} diversity? [→ **Yes**]{.fragment}
:::

::: {.fragment .fade-in}
-   Can [plankton]{style="color: black;background-color: #5ed496;border-radius: 10px"} diversity be predicted from [env]{style="color: black;background-color: #ffe50a;border-radius: 10px"}? [→ **Yes**]{.fragment}
:::
