...
[`ta_eve + ta_div + mo_ric ~ temperature + silicate + phosphate + oxygen + NPP + alkalinity + DIC`]{style="font-size: 50%;"}
...
Y_test <- all_preds %>% 
  filter(split == "test") %>% 
  select(c("ta_ric_3", "mo_ric", "ta_eve_1"))
...
[`POC_res ~ ta_eve_res + ta_div_res + mo_ric_res`]{style="font-size: 70%;"}
