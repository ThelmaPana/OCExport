...
#|output: false
source("utils.R")
load("data/07.att_from_plankton_select_fit.Rdata")
```
...
rsquares <- preds %>%
  ...
  rsq(truth = log_remain, estimate = .pred)
...
preds %>%
  ggplot() +
  geom_point(aes(x = log_remain, y = .pred, colour = cv_type), size = 0.5) +
...
repres_fold %>%
  ...
  ggplot() +
  geom_point(aes(x = log_remain, y = .pred, colour = cv_type)) +
  geom_abline(intercept = 0, slope = 1, colour = "red") +
  coord_fixed() + 
...
df_trends <- df %>% 
  ...
  select(lon, lat, log_remain, all_of(vars_pdp$variable))

df_trends_long <- df_trends %>% 
  select(-lon) %>% 
  pivot_longer(c(log_remain, all_of(vars_pdp$variable))) %>% 
  mutate(name = factor(name, levels = c("log_remain", vars_pdp$variable)))
...
colour_scales <- c("asc", "asc", "div", "div")
