...
#| cache: false
source("utils.R")
load("data/10.all_preds_poc_from_plankton_best.Rdata")
rm(all_preds, xgb_explain)
output_filename <- "data/12.all_preds_plankton_best_from_env.Rdata"
```
...
#resp_vars <- c("ta_ric_3", "mo_ric", "ta_eve_1")
resp_vars <- vars_pdp
...
::: {.callout-caution appearance="simple" icon="false"}
TODO: try to predict `sqrt(mo_ric)` and `sqrt(ta_mast)`
:::
...
# also predict training set to save for lates
Y_train_pred = MBT_model.predict(X_train, n=best_params['n_boosts'])
...
## Training set
# Predictions
train_preds <- py$Y_train_pred %>% 
  as_tibble(.name_repair = "minimal") %>% 
  set_names(paste0("pred_", resp_vars)) %>% 
  mutate(split = "train")

# Residuals
train_res <- py$Y_train - py$Y_train_pred %>% 
  as_tibble(.name_repair = "minimal") %>% 
  set_names(paste0("resid_", resp_vars)) 

# Store together with meta and response variables
train <- df_train %>% 
...
## Test set
# Predictions
test_preds <- py$Y_test_pred %>% 
  as_tibble(.name_repair = "minimal") %>% 
  set_names(paste0("pred_", resp_vars)) %>% 
  mutate(split = "test")

# Residuals
test_res <- py$Y_test - py$Y_test_pred %>% 
  as_tibble(.name_repair = "minimal") %>% 
  set_names(paste0("resid_", resp_vars)) 
...
all_preds <- bind_rows(train, test)
