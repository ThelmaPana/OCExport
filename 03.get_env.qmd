...
```{r}
gg_map(par_1 %>% filter(month == 1), "par", type = "raster")
```
...
```{r}
gg_map(env_m, "temperature", type = "raster") + facet_wrap(~month, ncol = 3)
```
...
```{r}
gg_map(env_m %>% filter(month == 1), "temperature", type = "raster", land = FALSE)
gg_map(env_m %>% filter(month == 1), "par", type = "raster", land = FALSE)
gg_map(env_m %>% filter(month == 1), "mld", type = "raster", land = FALSE)
gg_map(env_m %>% filter(month == 1), "mld_argo", type = "raster", land = FALSE)
```
...
```{r}
save(env, file = "data/03.all_env.Rdata")
```
...
plot_list <- list()
for (i in 1:length(to_plot)){plot_list[[i]] <- gg_map(env, to_plot[i], type = "raster")}
