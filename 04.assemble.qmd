...
```{r}
#df <- df %>% select(poc, lon, lat, contains("ta_"), contains("mean"))
```
...
```{r}
# Need to remove sample_ids, lon and lat and to scale because units differ between variables
pca_all <- FactoMineR::PCA(df %>% select(-c(sample_ids, lon, lat)), scale.unit = TRUE, graph = FALSE)
```
...
-   PC2: deep mixed layer in positive values
