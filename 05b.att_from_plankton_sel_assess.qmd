---
title: "Prediction assessment of POC attenuation from selected plankton metrics"
subtitle: "Assess the XGBoost model to predict POC attenuation values from plankton diversity data."
...
#|output: false
source("utils.R")
load("data/05.att_from_plankton_sel_fit.Rdata")
```
...
v_lines <- full_vip %>% 
  filter(variable == "_full_model_") %>% 
  group_by(cv_type) %>% 
  summarise(base_rmse = mean(dropout_loss))
...
full_vip %>%
  ...
  ggplot() +
  geom_vline(data = v_lines, aes(xintercept = base_rmse), colour = "grey", linewidth = 2) +
  geom_boxplot(aes(x = dropout_loss, y = variable, colour = cv_type)) +
...
colour_scales <- c("asc", "asc", "div", "asc")
