...
load("data/08.all_preds_poc_from_env.Rdata")
poc_res <- all_preds %>% select(lon, lat, datetime, resid_poc = residuals)

# plankton residuals from env
load("data/12.all_preds_plankton_best_from_env.Rdata")
plankton_res <- all_preds %>% select(lon, lat, datetime, contains("resid"))

# store together
df <- left_join(poc_res, plankton_res, by = join_by(lon, lat, datetime))

output_filename <- "data/13.all_preds_poc_res_from_plankton_res.Rdata"
```
...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(-resid_poc) %>%  select(contains("resid")) %>% colnames()
# Metadata variables
