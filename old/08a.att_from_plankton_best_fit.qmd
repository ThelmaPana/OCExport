...
output_filename <- "data/11.poc_from_plankton_best_fit.Rdata"
```
...
```{r poc_hist}
df %>% ggplot() + geom_histogram(aes(x = poc_1000), bins = 100)
```
...
```{r poc_log_hist}
df <- df %>% mutate(poc_log = log(poc_1000))
...
```{r roles}
# Response variable
resp_var <- c("poc_log", "poc_1000") # keep both untransformed and transformed predictor
# Explanatory variables
exp_vars <- c("ta_mast", "ta_ric_3", "mo_eve")
# Metadata
meta_vars <- c("lon", "lat")
```
...
xgb_form <- as.formula(paste("poc_log ~ ", paste(c("poc_1000", exp_vars), collapse = " + "), sep = ""))
```
...
res <- mclapply(1:nrow(folds), function(i){
  ...
  xgb_rec <- recipe(xgb_form, data = df_train) %>%
    update_role(poc_1000, new_role = "untransformed outcome")
