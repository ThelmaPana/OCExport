...
#|cache: false
source("utils.R")
load("data/11.poc_from_plankton_best_fit.Rdata")
```
...
full_vip <- res %>%
  ...
  mutate(variable = forcats::fct_reorder(variable, dropout_loss, .desc = TRUE))

# Variable importance across folds
full_vip %>% 
  # For each fold in each cv_type
  group_by(cv_type, fold) %>% 
  # compute the decrease in R² compared to full model for each variable
  mutate(loss_rsq = max(dropout_loss) - dropout_loss) %>% 
  filter(variable != "_full_model_") %>% 
  ggplot() + 
  geom_vline(data = full_vip %>% filter(variable == "_full_model_"), aes(xintercept = mean(dropout_loss)), colour = "grey", linewidth = 2) +
  geom_boxplot(aes(x = loss_rsq, y = variable, colour = cv_type)) +
  labs(x = "Loss in R² after removing predictor", y = "Predictor") +
...
full_vip %>%
  ...
  summarise(loss_rsq = mean(loss_rsq), .groups = "drop") %>%
  ggplot() +
  geom_vline(data = full_vip %>% filter(variable == "_full_model_"), aes(xintercept = mean(dropout_loss)), colour = "grey", linewidth = 2) +
  geom_boxplot(aes(x = loss_rsq, y = variable, colour = cv_type)) +
  labs(x = "Loss in R² after removing predictor", y = "Predictor")
...
mean_pdp %>% 
  ...
  filter(var_name != "ta_mast") %>% 
ggplot() + 
  geom_path(aes(x = x, y = yhat_loc), color = "darkblue") +
