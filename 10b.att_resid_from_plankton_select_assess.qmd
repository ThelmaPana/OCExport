---
title: "Prediction assessment POC remaining residuals values from plankton"
subtitle: "Train a XGBoost model to predict POC remaining residuals values from plankton diversity data."
...
#|output: false
source("utils.R")
load("data/10.att_resid_from_plankton_select_fit.Rdata")
```
...
rsquares <- preds %>%
  ...
  rsq(truth = resid_remain, estimate = .pred)
...
preds %>%
  ggplot() +
  geom_point(aes(x = resid_remain, y = .pred, colour = cv_type), size = 0.5) +
...
repres_fold %>%
  ...
  ggplot() +
  geom_point(aes(x = resid_remain, y = .pred, colour = cv_type)) +
  geom_abline(intercept = 0, slope = 1, colour = "red") +
  coord_fixed() + 
...
df_trends <- df %>% 
  select(lon, lat, resid_remain, all_of(vars_pdp$variable))

df_trends_long <- df_trends %>% 
  select(-lon) %>% 
  pivot_longer(c(resid_remain, all_of(vars_pdp$variable))) %>% 
  mutate(name = factor(name, levels = c("resid_remain", vars_pdp$variable)))
...
colour_scales <- c("div", "div", "div", "asc")
