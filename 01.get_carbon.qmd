...
# Save global values of b
save(argo_map_coef, file = "data/01.argo_map_coef.Rdata")
```
...
First, join UVP profiles with canyon profiles by rounding coordinates
...
# Round UVP lon and lat to match POC dataset
uvp_prof <- uvp_prof %>%
  #select(profile_id, lon, lat) %>% 
  mutate(
    lon_r = roundp(lon, precision = 0.25, f = floor) + 0.125,
    lat_r = roundp(lat, precision = 0.25, f = floor) + 0.125
  ) 

# Get POC profiles
can_prof <- df_can %>% 
  select(pix_id, lon_r = lon, lat_r = lat) %>% 
  unique()

# Join UVP and POC profiles on rounded coordinates
uvp_prof <- uvp_prof %>% 
  left_join(can_prof, by = join_by(lon_r, lat_r))

sum(is.na(uvp_prof$pix_id))

ggplot(uvp_prof) +
  geom_polygon(data = world, aes(x = lon, y = lat, group = group), fill = "grey") +
  geom_point(aes(x = lon, y = lat, colour = is.na(pix_id)), size = 0.5) +
  labs(title = "UVP-canyon matching", colour = "Matched") +
  coord_quickmap(expand = 0)
```

Some UVP profiles could not be matched because there is no corresponding data in the canyon dataset. Let’s ignore these UVP profiles. For profiles that could be matched, we extract the corresponding pixels in the canyon dataset and compute attenuation.

```{r b_uvp}
# Extract matched canyon pixels
can_pix <- uvp_prof %>% drop_na(pix_id) %>% select(pix_id)

# Keep only these pixels
df_can_uvp <- df_can %>% filter(pix_id %in% can_pix$pix_id)
...
# Add info regarding reference depth and POC at reference depth
argo_uvp_coef <- argo_uvp_coef %>% 
  rename(att = b) %>% 
  left_join(
    df_can_uvp %>% 
     filter(depth == z0) %>% 
     select(lon, lat, z0, poc0 = poc),
    by = join_by(lon, lat)
  )

# Join with UVP profiles
uvp_prof_c <- uvp_prof %>% 
  left_join(argo_uvp_coef %>% rename(lon_r = lon, lat_r = lat), by = join_by(lon_r, lat_r))
...
ggplot(uvp_prof_c, aes(x = lat, y = att)) +
  geom_point(size = 0.5) +
  geom_smooth() +
  coord_flip()
```
...
-   out of the `r nrow(uvp_prof)` UVP5 profiles, POC attenuation could be computed for `r sum(!is.na(uvp_prof$att))` of them.

-   this corresponds to `r nrow(uvp_prof %>% drop_na(att) %>% select(pix_id) %>% distinct())` unique POC profiles on the 0.25×0.25° grid.

## Save

```{r save}
save(uvp_prof_c, file = "data/01.uvp_poc.Rdata")
```
