...
inds <- mo_space$ind$coord %>% as_tibble()
...
p12 <- ggmorph_tile(mo_space, o$path_to_img, steps = 18, n_imgs = 3, fun = preprocess, dimensions = c(1,2), scale = 0.02) 
p12 + 
...
p34 <- ggmorph_tile(mo_space, o$path_to_img, steps = 18, n_imgs = 3, fun = preprocess, dimensions = c(3,4), scale = 0.02) 
p34 + 
  geom_path(data = circ, aes(x = x*k, y = y*k), lty = 2, color = "grey", alpha = 0.7) + 
  geom_hline(yintercept = 0, color="grey", alpha = 0.9) +
  geom_vline(xintercept = 0, color="grey", alpha = 0.9) +
...
o %>%
  ggplot(aes(x = mo_size, y = mo_dim2, colour = morph)) +
  geom_point(show.legend = FALSE, size = 0.5, alpha = 0.05)
```
...
morpho_div <- morpho_div$functional_diversity_indices %>% 
  ...
  as_tibble() %>% 
  select(profile_id, mo_ric = fric, mo_div = fdiv, mo_eve = feve)
...
profiles <- profiles %>% 
  rename(
    mo_size_mean = mo_dim1_mean,
    mo_grey_mean = mo_dim2_mean,
    mo_elon_mean = mo_dim3_mean,
    mo_siz_mean  = mo_dim4_mean,
    mo_size_sd = mo_dim1_sd,
    mo_grey_sd = mo_dim2_sd,
    mo_elon_sd = mo_dim3_sd,
    mo_siz_sd  = mo_dim4_sd,
  )
```
