...
-   [TODO: tentacles of Cnidaria?]{style="color: red;"}
...
```{r}
# Compute mean and variance of dim1, dim2 and dim3 per profile
m_div_prof <- o %>% 
  group_by(profile_id, lon, lat) %>% 
  summarise(across(m_dim1:m_dim3, list(mean = mean, var = var))) %>% 
  ungroup()
