...
exp_vars <- c("mo_ric", "ta_mast", "ta_ric_3")
# Metadata
meta_vars <- c("lon", "lat")
```
...
```{r}
model_parts(xgb_explain, loss_function = loss_yardstick(yardstick::rsq), type = "raw") %>%
    bind_rows() #%>%
    #filter(variable != "_baseline_")
model_parts(xgb_explain, loss_function = loss_yardstick(yardstick::rsq), type = "ratio") %>%
    bind_rows() #%>%
