---
title: "Predict POC remaining values from plankton"
subtitle: "Train a XGBoost model to predict POC remaining values from PCAd plankton diversity data."
...
output_filename <- "data/11.att_from_plankton_pca_fit.Rdata"
```
...
```{r poc_hist}
df %>% ggplot() + geom_histogram(aes(x = remain), bins = 100)
```

The distribution is not normal, this is problematic. Let’s try a log-transformation.

```{r log_remain_log_hist}
df <- df %>% mutate(log_remain = log(remain))
df %>% ggplot() + geom_histogram(aes(x = log_remain), bins = 100)
```

This is much better. We’ll then try to predict `log(remain)`.

### Variable roles

```{r roles}
# Response variable
resp_var <- c("remain", "log_remain") # keep both untransformed and transformed predictor
# Explanatory variables
#exp_vars <- df %>% select(contains("mo")) %>% colnames()
exp_vars <- df %>% select(prop_crustacea:mo_spe) %>% colnames()
#exp_vars <- df %>% select(abund, ta_ric_1, ta_eve_1, ta_div_1, ss_slope, mo_ric, mo_eve, mo_div, mo_size_mean, mo_grey_mean) %>% colnames()
# Metadata
meta_vars <- c("lon", "lat")
```
...
plot(exp_pca, choix = "var", axes = c(1, 2))
plot(exp_pca, choix = "var", axes = c(2, 3))
...
folds <- nested_cv(
  df,
  outside = vfold_cv(v = n_folds, strata = log_remain, breaks = 4),
  inside = vfold_cv(v = n_folds, strata = log_remain, breaks = 4)) %>%
  mutate(cv_type = "stratified")
```
...
xgb_form <- as.formula(paste("log_remain ~ ", paste(c("remain", exp_vars), collapse = " + "), sep = ""))
```
...
res <- mclapply(1:nrow(folds), function(i){
  ...
  preds <- predict(final_res, new_data = df_test) %>%
    bind_cols(df_test %>% select(log_remain))
