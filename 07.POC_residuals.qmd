---
title: "Compute POC anomalies"
subtitle: "Predict POC values from environmental data and extract the residuals."
format:
  html:
    toc: true
editor: visual
...
Select explanatory variables as in Wang et al. 2023.

```{r}
# Explanatory variables
#exp_vars <- df %>% select(temperature_mean:par_sd) %>% select(-nitrate_mean) %>% colnames()
exp_vars <- df %>% select(
  temperature_mean, temperature_sd,
  silicate_mean, silicate_sd,
  phosphate_mean, phosphate_sd,
  z_eu_mean, z_eu_sd,
  oxygen_mean, oxygen_sd
  ) %>% colnames()
# Metadata variables
...
Have a look at residuals on test set.
...
```{r}
poc_res <- augment(final_res, new_data = df) %>% 
  rename(.pred_logged = .pred) %>%
  mutate(.pred = exp(.pred_logged)) %>%
  mutate(residuals = .pred_logged - poc_log) %>% 
  select(lon, lat, datetime, residuals)

save(poc_res, file = "data/07.poc_res.Rdata")
```
...
ggplot_pdp(xgb_pdp_phos, phosphate_sd) +
