---
title: "Predict POC attenuation from plankton"
subtitle: "Train a XGBoost model to predict POC attenuation from selected plankton diversity data."
...
output_filename <- "data/05.att_from_plankton_sel_fit.Rdata"
```
...
```{r poc_hist}
df_all %>% ggplot() + geom_histogram(aes(x = att), bins = 100)
```

The distribution is \~ normal, let’s keep it this way.

### Variable roles

```{r roles}
# Response variable
resp_var <- c("att") # keep both untransformed and transformed predictor
# Explanatory variables (keep all initially, then select)
exp_vars <- df_all %>% select(prop_crustacea:mo_spe) %>% colnames()
# Metadata
meta_vars <- c("lon", "lat")
```
...
Use Escoufier’s equivalent vectors to select a few predictors that are representative of the entire plankton dataset.

```{r pred_sel}
e <- escouf(df_all %>% select(all_of(exp_vars)), verbose = FALSE)
plot(e, main = "Escoufier's equivalent vectors for plankton descriptors")
var_thr <- 0.8 # proportion of variance to retain from the original dataset
exp_vars <- extract(e, level = var_thr) %>% colnames()
```
...
-   stratified on deciles of the response variable
...
df_all_sf <- st_as_sf(df_all, coords = c("lon", "lat"), crs = 4326)
...
folds <- bind_rows(
  ...
  nested_cv(
    df_all,
    outside = vfold_cv(v = n_folds, strata = att, breaks = 9),
    inside = vfold_cv(v = n_folds, strata = att, breaks = 9)) %>%
...
-   `mtry`: number of predictors to select for each tree

-   `learn_rate`: learning rate

NB: `sample_size` is set at 0.5, meaning that we use only 50% of observations for each tree.
...
xgb_form <- as.formula(paste("att ~ ", paste(exp_vars, collapse = " + "), sep = ""))
```
...
xgb_grid <- grid_latin_hypercube(
  trees(range = c(300, 3000)),
  learn_rate(range = c(-6, -1), trans = log10_trans()),
  tree_depth(range = c(1, 10)),
  mtry(c(1, length(exp_vars))),
  size = 30
)
```
