...
subtitle: "Train a XGBoost model to predict POC values from few variables of plankton diversity data."
...
output_filename <- "data/07.all_preds_poc_from_plankton_best.Rdata"
```
...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(ta_div, mo_div, ta_eve, mo_eve) %>% colnames()
# Metadata variables
...
```{r pdp}
xgb_pdp_ta_eve <- model_profile(explainer = xgb_explain, variables = c("ta_eve"))
ggplot_pdp(xgb_pdp_ta_eve, ta_eve) +
  labs(x = "Taxonomic Eveness", y = "Logged predicted POC")

xgb_pdp_ta_div <- model_profile(explainer = xgb_explain, variables = c("ta_div"))
ggplot_pdp(xgb_pdp_ta_div, ta_div) +
  labs(x = "Taxonomic Diversity", y = "Logged predicted POC")

xgb_pdp_mo_eve <- model_profile(explainer = xgb_explain, variables = c("mo_eve"))
ggplot_pdp(xgb_pdp_mo_eve, mo_eve) +
  labs(x = "Morphological Eveness", y = "Logged predicted POC")

xgb_pdp_mo_div <- model_profile(explainer = xgb_explain, variables = c("mo_div"))
ggplot_pdp(xgb_pdp_mo_div, mo_div) +
  labs(x = "Morphological Divergence", y = "Logged predicted POC")
```

## Conclusion

Using only `r length(exp_vars)` variables describing plankton diversity, we can explain `r percent(round(rsq(test_preds, truth = poc_log, estimate = .pred_logged)$.estimate, digits = 2))` of POC variance.
