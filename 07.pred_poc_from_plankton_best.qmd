...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(ta_eve, ta_div, mo_ric) %>% colnames()
# Metadata variables
...
xgb_pdp_mo_ric <- model_profile(explainer = xgb_explain, variables = c("mo_ric"))
ggplot_pdp(xgb_pdp_mo_ric, mo_ric) +
  labs(x = "Morphological Richness", y = "Logged predicted POC")
```
