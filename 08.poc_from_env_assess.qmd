---
title: "Prediction assessment of POC values from env"
...
#|cache: false
source("utils.R")
load("data/07.poc_from_env_fit.Rdata")
```
...
We can alto take an average weighted by R².

```{r w_rsq_var_imp_mean}
vip_mean_rsq <- full_vip %>% 
  left_join(rsquares %>% select(cv_type, fold, rsq = .estimate), by = join_by(cv_type, fold)) %>% 
  filter(variable != "_full_model_") %>%
  group_by(cv_type, fold, variable) %>% 
  summarise(dropout_loss = wtd.mean(dropout_loss, weights = rsq, na.rm = TRUE))

ggplot(vip_mean_rsq) +
  geom_vline(data = full_vip %>% filter(variable == "_full_model_"), aes(xintercept = mean(dropout_loss)), colour = "grey", linewidth = 2) +
  geom_boxplot(aes(x = dropout_loss, y = variable, colour = cv_type)) +
  labs(x = "Weighted (R²) mean RMSE after permutations across CV folds")
```

Or we can also compute a weighted average with 1/var as weights.

```{r w_var_var_imp_mean}
vip_mean_var <- full_vip %>% 
  filter(variable != "_full_model_") %>%
  group_by(cv_type, fold, variable) %>% 
  mutate(var_d_loss = var(dropout_loss)) %>% 
  summarise(
    dropout_loss = wtd.mean(dropout_loss, weights = 1/var_d_loss, na.rm = TRUE)
  )
...
How do we average variable importance across folds?

-   simple average

-   weighted average by R²

-   weighted average by 1/sd

All are very similar but actually different (values were checked!).
:::
