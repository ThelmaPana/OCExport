...
subtitle: "Train a XGBoost model to predict POC values from a set of PCA-selected variables of plankton diversity."
...
output_filename <- "data/08.all_preds_poc_from_plankton_pca.Rdata"
```
...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(ta_ric:mo_spe) %>% colnames()
# Metadata variables
...
df_pca <- df %>% select(poc, poc_log, all_of(exp_vars))
...
Let’s keep first 3 PCs to predict POC values.
...
We will use projections of profiles in the first plane of the PCA as predictors.

```{r pc_as_preds}
# Replace original data by data with PC projections
df <- inds

# Define new set of exp variables
exp_vars <- df %>% select(dim1, dim2, dim3) %>% colnames()
```
...
xgb_form <- as.formula(paste("poc_log ~ ", paste(c("poc", meta_vars, exp_vars), collapse = " + "), sep = ""))
...
```{r eval_test}
rmse(test_preds, truth = poc_log, estimate = .pred_logged)
rsq(test_preds, truth = poc_log, estimate = .pred_logged)
```
...
save(all_preds, file = "data/07.all_preds_poc_from_plankton_best.Rdata")
```
...
```{r pdp}
xgb_pdp_dim1 <- model_profile(explainer = xgb_explain, variables = c("dim1"))
ggplot_pdp(xgb_pdp_dim1, dim1) +
  labs(x = "Dim 1", y = "Logged predicted POC")

xgb_pdp_dim2 <- model_profile(explainer = xgb_explain, variables = c("dim2"))
ggplot_pdp(xgb_pdp_dim2, dim2) +
  labs(x = "Dim 2", y = "Logged predicted POC")

xgb_pdp_dim3 <- model_profile(explainer = xgb_explain, variables = c("dim3"))
ggplot_pdp(xgb_pdp_dim3, dim3) +
  labs(x = "Dim 2", y = "Logged predicted POC")
```

## Conclusion

Using only the first 3 axes of a PCA fed with variables describing plankton diversity, we can explain `r percent(round(rsq(test_preds, truth = poc_log, estimate = .pred_logged)$.estimate, digits = 2))` of POC variance.
