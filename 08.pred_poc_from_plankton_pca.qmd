...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(ta_ric:mo_eve) %>% colnames()
# Metadata variables
...
Let’s keep first 2 PCs to predict POC values.
...
exp_vars <- df %>% select(dim1, dim2) %>% colnames()
```
...
Using only the first two axes of a PCA fed with variables describing plankton diversity, we can explain `r percent(round(rsq(test_preds, truth = poc_log, estimate = .pred_logged)$.estimate, digits = 2))` of POC variance.
