...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(ta_ric:par_sd) %>% colnames()
# Metadata variables
...
xgb_pdp_phos <- model_profile(explainer = xgb_explain, variables = c("phosphate_sd"))
ggplot_pdp(xgb_pdp_phos, phosphate_sd) +
  labs(x = "Phospate_sd", y = "Logged predicted POC")
