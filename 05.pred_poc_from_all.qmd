---
title: "Predict POC values from both env and plankton"
subtitle: "Train a XGBoost model to predict POC values from environmental data and plankton diversity data."
...
output_filename <- "data/05.all_preds_poc_from_all.Rdata"
```
...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(ta_ric:par) %>% colnames()
# Metadata variables
...
```{r pdp}
xgb_pdp_1 <- model_profile(explainer = xgb_explain, variables = c("z_eu"))
ggplot_pdp(xgb_pdp_1, z_eu) +
  labs(x = "z_eu", y = "Logged predicted POC")

xgb_pdp_2 <- model_profile(explainer = xgb_explain, variables = c("dic"))
ggplot_pdp(xgb_pdp_2, dic) +
  labs(x = "DIC", y = "Logged predicted POC")

xgb_pdp_3 <- model_profile(explainer = xgb_explain, variables = c("nitrate"))
ggplot_pdp(xgb_pdp_3, nitrate) +
  labs(x = "Nitrate", y = "Logged predicted POC")
```
