...
load("data/08.plankton_res.Rdata")

df <- poc_res %>% 
  rename(poc_res = residuals) %>% 
  left_join(plankton_res %>% rename(plankton_res = residuals), by = join_by(lon, lat, datetime))
```
...
```{r}
# Explanatory variables
exp_vars <- df %>% select(plankton_res) %>% colnames()
# Metadata variables
