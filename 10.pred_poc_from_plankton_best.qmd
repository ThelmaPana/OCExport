...
load("data/07.all_preds_poc_from_plankton.Rdata")
best_vars <- vars_pdp # best plankton predictors
rm(all_preds, xgb_explain)
output_filename <- "data/10.all_preds_poc_from_plankton_best.Rdata"
```
...
```{r sel_var}
# Explanatory variables
exp_vars <- df %>% select(all_of(best_vars)) %>% colnames()
# Metadata variables
...
```{r save}
# Save all predictions, explainer and most important variables
save(all_preds, xgb_explain, vars_pdp, file = output_filename)
```
