...
subtitle: "Predict poc values from plankton diversity data."
format:
  html:
    toc: true
editor: visual
...
## Select only plankton variables

```{r}
df <- df %>% select(lon, lat, datetime, ta_ric:m_dim3_var, poc)
```
...
pca_all <- FactoMineR::PCA(df %>% select(-c(lon, lat, datetime)), scale.unit = TRUE, graph = FALSE)
```
...
```{r}
xgb_pdp_pielou <- model_profile(explainer = xgb_explain, variables = c("ta_pielou"))
ggplot_pdp(xgb_pdp_pielou, ta_pielou) +
  labs(x = "Pielou", y = "Logged predicted POC")

xgb_pdp_m_dim1_mean <- model_profile(explainer = xgb_explain, variables = c("m_dim1_mean"))
ggplot_pdp(xgb_pdp_m_dim1_mean, m_dim1_mean) +
  labs(x = "M dim1 mean", y = "Logged predicted POC")

xgb_pdp_shannon <- model_profile(explainer = xgb_explain, variables = c("ta_shannon"))
ggplot_pdp(xgb_pdp_shannon, ta_shannon) +
  labs(x = "Shannon", y = "Logged predicted POC")

xgb_pdp_m_dim2_var <- model_profile(explainer = xgb_explain, variables = c("m_dim2_var"))
ggplot_pdp(xgb_pdp_m_dim2_var, m_dim2_var) +
  labs(x = "M dim2 var", y = "Logged predicted POC")
```
